...
def search_by_varos(varos):
    """Allomasok varos szerinti keresese"""
    conn = get_db()
    try:
        cur = conn.cursor()
        try:
            # Az SQL lekerdezesben escape-eljuk a @ karaktert, igy az utana kovetkezo specialis karaktetek
            # normal karakterkent kerulnek feldolgozasra. Kicsereljuk a @, _ es % karakteret.
            varos = varos.replace("@", "@@")
            varos = varos.replace("%", "@%")
            varos = varos.replace("_", "@_")
            # A keresesi feltetel kiegeszitese, hogy illeszkedjen toredek mintara is.
            varos = "%" + varos + "%"
            # SQL lekerdezes
            cur.execute("SELECT id, nev, varos FROM allomas" +
                        " WHERE varos LIKE :var ESCAPE '@'", var=varos)
            # Eredmenytomb
            results = []
...
def search_by_nev(nev):
    """Allomasok nev szerinti keresese"""
    conn = get_db()
    try:
        cur = conn.cursor()
        try:
...
def get_id():
    """Segedfuggveny a Post metodushoz. Egyedi azonositot general az allomas szamara"""
    conn = get_db()
    try:
        cur = conn.cursor()
        try:
